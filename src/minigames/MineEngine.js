...
export class MineEngine {
  ...
  start(onComplete) {
    ...
    this.powerText = this.scene.add.text(bx, L.top + 170, 'Swing — hold SPACE, strike the glinting seam', { fontSize: '16px', fontFamily: FONT, color: COLOR.muted });
  ...
  strike() {
    ...
    if (power > c.overchargeAt) {
      this.lastMiss = 'wild';
      this._resolve('miss', 'WILD SWING', 0xff8c42, () => {
      ...
      this._resolve(perfect ? 'perfect' : 'good', perfect ? 'CLEAN BREAK!' : 'STRIKE', perfect ? 0xffffff : 0xffd700, () => {
      ...
      this._resolve('miss', 'GLANCING', 0x99a0b0, () => {
        this.fracture = clamp01(this.fracture + c.fracturePerStrike * c.glanceFractureMult);
  ...
  _layout() {
    ...
    this.fracText.setText(`Fracture  ${Math.round(this.fracture * 100)}%`);
    ...
    this.shockText.setText('Shock  (let it cool between blows)');
    ...
    this.stabText.setText('Stability');
    ...
    this.gearText.setText(this.gear === 'deep'
      ? 'Digging DEEP — fast, and it heats the face    [Left] ease off'
      : 'Digging SHALLOW — safe, and slow    [Right] bite deeper');
